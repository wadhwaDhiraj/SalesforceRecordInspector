...
export function activate(context: vscode.ExtensionContext) {
    ...
    async function launchInspector(text: string) {
        try {
            await vscode.window.withProgress({
                ...
                if (!objectName) throw new Error(`Unknown Key Prefix: ${text.substring(0,3)}`);
            ...
            let msg = err.message;
            if (msg.includes("NOT_FOUND")) msg = `Record ID ${text} does not exist.`;
    ...
    let logHistoryDisposable = vscode.commands.registerCommand('sfInspector.scanLogVariable', async (uri?: vscode.Uri) => {
        ...
        try {
            ...
            RecordPanel.createOrShowLogMode(
                context.extensionUri,
                varName,
                versions,
                prefixService,
